...
TEST_F(GameFixture, makeAndPlaceOverwrite) {
	EXPECT_FALSE(g.makeAndPlace("Spare", 'S', { 0, 0 })); //should be unable to overwrite
	EXPECT_EQ(g.getCell({ 0,0 })->getMapIcon(), 'P'); //should match original
}

TEST_F(GameFixture, missileLaunch) {
	g.makeAndPlace("City", 'C', { 2, 2 });
...
TEST_F(GameFixture, sortByDistance) {
	...
	Coord a2{ 1, 0 }; //1 away
	Coord a3{ 3, 0 }; //3 away
	g.makeAndPlace("A1", '1', a1);
	g.makeAndPlace("A2", '2', a2);
	g.makeAndPlace("A3", '3', a3);
